...
async function autoMigrateLocalMedia(actions) {
  ...
  for (const item of migrated) {
    ...
    for (const field of MEDIA_PATH_FIELDS) {
      ...
      // اسم ميديا سحابي اتقصّر (m-<sha>.<ext> بدون الرابط) — ده أصل محفوظ
      // على مستودع الميديا؛ مانحاولش نرفعه كملف محلي، نجيب رابطه الحقي.
      if (/^m-[0-9a-f]+\.[a-z0-9]+$/i.test(String(current).trim())) {
        try {
          const r = await api.media.resolve(String(current).trim());
          if (r && r.ok && r.url) {
            item[field] = r.url;
            changed = true;
          }
        } catch (e) {
          // تجاهل — نسيب القيمة كما هي لحد ما نجرب تاني
        }
        continue;
      }
...
function openActionModal(p47 = null) {
  ...
  document.getElementById("m-save").addEventListener("click", async () => {
    ...
    const vF3 = p54 => {
      ...
      if (!v66) return "";
      // data-value هي القيمة الحقيقية (الرابط السحابي بالكامل) — النص المعروض
      // في السبين بيتقصّر للعرض فقط، فماينفعش نقرأه ونحفظه (ده كان بيضيع الميديا)
      const real = v66.getAttribute && v66.getAttribute("data-value");
      if (real) return real;
      if (v66.textContent !== "No file selected") {
...
window.pickFile = async (p75, p76) => {
  ...
  if (openFileResult) {
    ...
    if (el) {
      // القيمة الحقيقية في data-value (الرابط الكامل) والنص المعروض مختصر
      el.setAttribute("data-value", finalPath || "");
      el.textContent = mediaDisplayPath(finalPath);
    }
  }
};
let hotkeysData = [];
      ...
      api.system.downloadSound(v524).then(async (r2) => {
        if (r2 && r2.ok && r2.path) {
          try {
            ...
            const chosen = finalPath || r2.path;
            v525.setAttribute("data-value", chosen || "");
            v525.textContent = mediaDisplayPath(chosen);
            vF27();
            // لا نعرض رسالة فورية عند حفظ الصوت من المكتبة.
          } catch (err) {
            v525.setAttribute("data-value", r2.path || "");
            v525.textContent = mediaDisplayPath(r2.path);
            vF27();
            uiAlert("تعذر إنهاء رفع الصوت للسحابة — تم حفظ النسخة المحلية بدلًا منها ⚠");
          }
        } else {
          v525.setAttribute("data-value", v524 || "");
          v525.textContent = mediaDisplayPath(v524);
          vF27();
