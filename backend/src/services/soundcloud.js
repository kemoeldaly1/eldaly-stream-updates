...
// ---------------------------------------------------------------- حالة معزولة لكل حساب
const states = new Map(); // accountKey -> {queue, current, history, points}
...
function S() {
  if (!states.has(accountKey)) {
    const st = {
      queue: [],
      current: null,
      history: loadJson(histFile(accountKey), []),
      points: new Map(Object.entries(loadJson(pointsFile(accountKey), {}))),
    };
    states.set(accountKey, st);
  }
  return states.get(accountKey);
}

// تبديل الحساب — كل حساب بيرجع لحالته المنفصلة (طابور وتاريخ ونقاط)
function setAccount(key) {
  const k = String(key || "default");
  if (k === accountKey) return;
  persist();
  accountKey = k;
  S();
  broadcastQueue();
...
function persist() {
  ...
  try {
    ...
    fs.writeFileSync(histFile(accountKey), JSON.stringify(st.history.slice(0, 200)), "utf8");
    fs.writeFileSync(pointsFile(accountKey), JSON.stringify(Object.fromEntries(st.points)), "utf8");
  } catch (e) {}
}
...
function getPoints(user) {
  return S().points.get(String(user).toLowerCase()) || 0;
}
...
function validateAdd(track, user) {
  ...
  if (S().queue.length >= (cfg.maxQueue ?? 20)) return "الطابور مليان — استنى لحد ما يفضى";
  const perUser = S().queue.filter((q) => q.requestedBy.toLowerCase() === user.toLowerCase());
  if (perUser.length >= (cfg.maxQueuePerUser ?? 2)) return "وصلت الحد الأقصى من الطلبات في الطابور";
  if (S().queue.some((q) => q.id === track.id)) return "الأغنية دي موجودة في الطابور بالفعل";
...
async function addTrack(user, query) {
  ...
  if (!S().current) {
    ...
    S().current = { track: first, startedAt: Date.now() };
  }
  broadcastQueue();
  return track;
}
...
function hasUserQueued(user) {
  return S().queue.some((q) => q.requestedBy.toLowerCase() === user.toLowerCase());
}
